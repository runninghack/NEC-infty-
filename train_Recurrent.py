...
from graphclassification.baseline_Recurrent import INECN
...
class IGNN_NECDGTModel(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_nb):
        ...
        loss_node_mse = F.mse_loss(F_hat, tnsr_to_batch(Ft))
        loss_edge_mse = F.binary_cross_entropy_with_logits(He_logits, tnsr_to_batch(Et))
        loss = 1 * loss_node_mse + 5 * loss_edge_mse
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ne_ratio', type=float, default=5.0, help='node loss and edge loss ratio')
    parser.add_argument('--log_path', type=str, default="logs/ER20_recurrent/", help='directory to save the logs')
